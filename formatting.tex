...
\thesissetup{
    ...
    title       = {MCP server implementation for RHOAI workbenches},
    TeXtitle    = {MCP server implementation for RHOAI workbench},
    keywords    = {MCP, RHOAI, Workbench, Server, Implementation},
    TeXkeywords = {MCP, RHOAI, Workbench, Server, Implementation},
    abstract    = {
      This thesis presents MCP server implementation for RHOAI workbenches. The approach is based on the use of MCP, a protocol that allows for the efficient execution of matrix operations on GPUs. The thesis also presents a new algorithm for SpMM that is more efficient than the existing algorithms. The thesis also presents a new algorithm for SpMM that is more efficient than the existing algorithms.
    },
}
